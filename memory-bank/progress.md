...
* Premium Showcase Page: A dedicated marketing feature showcase route (`/premium`) highlighting core AI functionalities with 3 detailed cards utilizing transparent mascot graphics, smooth hover elevations, and responsive grids, linked directly via a premium amber/gold action button in the main header navbar.
...
* Restore Raw Text / AI Version Toggle: A subtle, two-way toggle button in the note edit view allowing users to switch the text area back and forth between the note's original unorganized raw text and its AI-organized structured markdown version without modifying the database or triggering AI calls until saved. For notes without both versions, it falls back to showing only the "Restore raw text" button when content differs.
